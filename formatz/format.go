...
func (f Formatted) Format() string {
	...
	if f.TruncateLength > 0 && txtLength > int(f.TruncateLength) {
		if f.TruncateLeft {
			nested = anzi.TruncateLeftPrefix(nested, int(f.TruncateLength), f.TruncatedReplacement)
		} else if f.TruncateRight {
			nested = anzi.TruncateRightSuffix(nested, int(f.TruncateLength), f.TruncatedReplacement)
		} else {
			nested = anzi.TruncateMid(nested, int(f.TruncateLength), f.TruncatedReplacement)
		}
	}
	...
	return fmt.Sprintf("%v%s%v", f.F, nested, anzi.Reset)
}
...
func New(color anzi.Color, s any) *Formatted {
	f := Formatted{
		F:          color,
		Nested:     s,
		Formatting: true,
...
func Unformat(o any) string {
	...
	return anzi.Unformat(s)
}

func Sprintf(color anzi.Color, s string, objects ...any) string {
	var formattedObjects []any
	for _, obj := range objects {
		switch o := obj.(type) {
		case Formatted:
			formatted := fmt.Sprintf("%s%s%s", anzi.Reset, o.String(), color)
